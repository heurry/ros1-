...
public:
    ...
    {
        string input_topic = nh_.param<std::string>("input_image_topic", "/usb_cam/image_raw");
        ROS_INFO_STREAM("camera_split subscribing to: " << input_topic);
        image_sub_ = it_.subscribe(input_topic, 1, &CameraSplitter::imageCallback, this);
